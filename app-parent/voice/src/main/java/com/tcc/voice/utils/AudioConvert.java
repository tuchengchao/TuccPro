...
public class AudioConvert {  
	...
	/**
	 * 通用音频格式参数转换 
	 * @param inputFileName -输入音频文件路径
	 * @param outputFileName -输出音频文件路径
	 * @param audioCodec -音频编码 
	 * @param sampleRate -音频采样率 
	 * @param audioBitrate -音频比特率 
	 * @param audioChannels -音频通道数
	 */
    public static void convert(String inputFileName, String outputFileName, int audioCodec, int sampleRate, int audioBitrate,  
            int audioChannels) {  
        ...
        // 开启抓取器  
        if (start(grabber)) {  
            recorder = new FFmpegFrameRecorder(outputFileName, audioChannels);  
	 ...
	 * @param audioChannels -音频通道数
	 */ 
    public static void convert(File inputFile, String outputFileName, int audioCodec, int sampleRate, int audioBitrate,  
            int audioChannels) {  
        Frame audioSamples = null;  
        // 音频录制（输出地址，音频通道）  
        FFmpegFrameRecorder recorder = null;  
    ...
    public static void main(String[] args) {
    	convert("F:\\2_1.wav", "F:\\20180122_convert.wav", avcodec.AV_CODEC_ID_PCM_S16LE, 16000, 256 * 1000,1);
    //	convert("F:\\2100.wav", "eguid.mp3", avcodec.AV_CODEC_ID_MP3, 8000, 16000,1); 
	}
}  