(function () {
    ...
    tw.cv = function(){
    	...
    	function getCv(event){
    		var img = cv.img, callback = cv.callback;
			$(".canvas").remove();
			$(".canvasBar").remove();
		...
		var listenCutKey = function(event){
			var e = event || window.event || arguments.callee.caller.arguments[0];
		...
		function preview(img, sel) {
			...
			var bar = $(".canvasBar"), vt = sel.height + sel.y1, vl = sel.width + sel.x1; 
			if(vt + bar.height() >= tw.ui.height){
				vt = vt - bar.height();
			}
			if(vl + bar.width() >= tw.ui.width){
				vl = vl - bar.width();
			}
			$(".canvasBar").css({
				top: vt,
    	...
    	_cv.bind = function(btn, img, beforecall, callback){
    		...
    		cv.btn = btn;
    		cv.img = img;
    		cv.beforeCall = beforecall;
    		cv.callback = callback;
    	}
    	_cv.keyBind = function (){
    		$(document).keyup(listenCutKey);
		} 
    	return _cv;
    }();
})();